...
with opaque dict payloads → two `msgspec` tagged unions**, 24 server tags and
7 client tags, both discriminated on `t`.
...
editing, the copilot RPC into the host page, and a token counter whose
client atom was written and never read. Those names are gone, and the
...
server.py     ServerMsg  — 24 branches, tag_field="t"
client.py     ClientMsg  — 7 branches, tag_field="t"
...
37 old events → 24 tags (four collapsed pairs, three new messages,
thirteen names retired with the features behind them).
