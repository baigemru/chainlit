...
``registry.py``    the live sessions, keyed by the conversation each is in --
                   one session a thread -- plus the policy that reads them.
                   It imports nothing from the rest of ``chainlit`` and
                   nothing from the transport, so it can be tested and
                   swapped on its own, which matters because it is the one
                   thing here that would have to become shared storage if
                   this ever ran on more than one process.
...
``handshake.py``   what ``hello`` means: the claim on a conversation -- kept
                   or begun, never refused -- and the replay that rebuilds
                   the client's screen.
