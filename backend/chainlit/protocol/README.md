...
| `resume_thread_error`  | **retired**                         | folded into `error`, then retired with its code: a resume that misses is answered by `session.ready` naming a different thread                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `set_sidebar_title`    | `sidebar.state`                     | **Collapsed pair, then promoted to a state.** The client reconciled both into one `sideView` atom, each handler reading the other's half out of the previous state — and neither could say "put the panel away and keep what is in it", which is why closing it destroyed the contents.                                                                                                                                                                                                                                                                      |
| `set_sidebar_elements` | `sidebar.state`                     | The panel is now `session.sidebar` — slots, an active one, and `visible` — and this frame is its projection: full, idempotent, and sent again on every reconnect and every refused operation. Slots name their elements **by id**; the elements travel ahead of it as `element.upsert` on the same FIFO queue. `key` is gone: its only power was to forbid updates, and identity is a property of the element's id now.                                                                                                                                      |
...
| —                       | `sidebar.user`  | **New.** What the user did to the element panel: `hide`, `show`, `activate`, `close`, `preview`. The half the old wire never had — "closed" used to be spelled by the client throwing its own copy away, and "open it again" could not be spelled at all. Scalar moves (`hide`/`show`/`activate`) are applied in silence **while the `rev` the frame quotes is still the panel's**; `close`, `preview`, every refusal and every stale `rev` answer with the whole `sidebar.state`. |
