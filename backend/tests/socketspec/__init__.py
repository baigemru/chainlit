...
There is one driver today (``legacy``, the socket.io handlers) and there will
be a second after the switchover, running the same table.
...
Everything here except ``legacy`` is free of ``chainlit`` imports, and a test
asserts it. That boundary is the whole deliverable: it is what lets the table
survive the transport it was written against.
"""
